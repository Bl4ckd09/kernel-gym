...
@jit
def _swiglu_kernel(a_ptr, b_ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    pid = tl.program_id(0)
    ...
    out = a * sig * b
    tl.store(out_ptr + offs, out.to(a_ptr.dtype.element_ty), mask=mask)
...
def solution(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    _swiglu_kernel[grid](a, b, out, n_elements)
    return out
