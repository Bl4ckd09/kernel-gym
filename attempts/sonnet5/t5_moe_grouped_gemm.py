...
def _grouped_gemm_kernel(
    ...
    row_start = tl.load(tile_row_start + pid_m)
    row_count = tl.load(tile_row_count + pid_m)

    offs_m = tl.arange(0, BLOCK_M)
    offs_n = pid_n * BLOCK_N + tl.arange(0, BLOCK_N)
    offs_k = tl.arange(0, BLOCK_K)

    row_mask = offs_m < row_count
    col_mask = offs_n < N

    x_ptrs = X + (row_start + offs_m)[:, None] * stride_xm + offs_k[None, :] * stride_xk
    w_ptrs = W + expert * stride_we + offs_k[:, None] * stride_wk + offs_n[None, :] * stride_wn
    ...
    out_ptrs = Out + (row_start + offs_m)[:, None] * stride_om + offs_n[None, :] * stride_on
...
def solution(sorted_x, weights, group_sizes):
    ...
    experts, tile_m, total = _tile_schedule(group_sizes)

    out = torch.empty((M, N), device=device, dtype=sorted_x.dtype)
    if total == 0:
        return out

    gs = group_sizes.to(torch.int64)
    group_starts = torch.cumsum(gs, 0) - gs  # exclusive prefix sum, per expert

    experts_l = experts.to(torch.int64)
    tile_m_l = tile_m.to(torch.int64)

    tile_row_start = (group_starts[experts_l] + tile_m_l * BLOCK_M).to(torch.int32).contiguous()
    tile_row_count = torch.clamp(gs[experts_l] - tile_m_l * BLOCK_M, min=0, max=BLOCK_M).to(torch.int32).contiguous()
    tile_expert = experts.contiguous()

    BLOCK_N = 64
    BLOCK_K = 32

    grid = (total, cdiv(N, BLOCK_N))
